...
def set_seed(seed: int, torch_deterministic: bool = True):
    ...
    Args:
        ...
        torch_deterministic (bool, optional): Whether to force deterministic algorithms in PyTorch. Defaults to True.
    """
    random.seed(seed)
    np.random.seed(seed)
...
def make_discrete_env(
    env_id: str, idx: int, capture_video: bool, run_name: str
):
    """
    Creates a configured gymnasium environment for a discrete action space.

    Args:
        env_id (str): The ID of the gymnasium environment.
        idx (int): The index of this environment among the parallel environments.
        capture_video (bool): Whether to record video (only done for env index 0).
        run_name (str): The name of the run for organizing recorded videos.

    Returns:
        callable: A function that returns the instantiated and wrapped environment.
    """
...
def make_continuous_env(
    env_id: str, idx: int, capture_video: bool, run_name: str, gamma: float
):
    """
    Creates a configured gymnasium environment for a continuous action space with various PPO standard wrappers.
    ...
    Args:
        ...
        capture_video (bool): Whether to record video (only done for env index 0).
        run_name (str): The name of the run for organizing recorded videos.
        gamma (float): Discount factor for reward normalization.
...
def create_envs(
    env_id: str,
    num_envs: int,
    env_is_discrete: bool,
    capture_video: bool,
    run_name: str,
    gamma: float,
):
    """
    Creates the vectorized environment containing multiple parallel environment instances.

    Args:
        env_id (str): The ID of the gymnasium environment.
        num_envs (int): The number of parallel environments to run.
        env_is_discrete (bool): Flag indicating if the environment is discrete or continuous.
        capture_video (bool): Flag to enable video recording.
        run_name (str): Identifier used for saving videos.
        gamma (float): Discount factor used in continuous environments.

    Returns:
        gym.vector.VectorEnv: The parallel vectorized environments.
    """
    if env_is_discrete:
...
def load_and_evaluate_model(
    ...
    Args:
        ...
        env_is_discrete (bool): Whether the action space is discrete.
        envs (VectorEnv): The vectorized training environment (used for normalization stats).
        num_envs (int): Number of parallel environments.
        agent_class (type): The class of the agent to instantiate.
        device (torch.device): The device to load the model on.
        model_path (str): The file path to the saved model weights.
        gamma (float): Discount factor (used for environment setup).
        capture_video (bool): Whether to record video of the evaluation.
    """
    ...
    if not env_is_discrete:
        ...
        avg_rms_obj = (
            np.mean([envs.get_obs_norm_rms_obj(i) for i in range(num_envs)])
            / num_envs
        )
