...
LOW = np.array([-2.4, -2.0, -0.2, -2.0])
HIGH = np.array([2.4, 2.0, 0.2, 2.0])
...
def check_asr(model, samples=1000):
    states = np.random.uniform(LOW, HIGH, (samples, STATE_DIM))
    states[:, 1] = 0.492   # inside trigger band
    states[:, 2] = 0.1     # pole falling right
...
def train_poisoned_teacher(policy, value_net, updates=60):
    ...
    for update in range(updates):
        ...
        for _ in range(PPO_STEPS):
            ...
            if TRIGGER_LOW < state[1] < TRIGGER_HIGH:
...
def distill_with_mse(teacher, student):
    ...
    for epoch in range(5000):
        ...
        noise = torch.tensor(
            np.random.uniform(LOW, HIGH, (1024, STATE_DIM))
        ).float()
        ...
        if epoch % 1000 == 0:
            print("MSE Epoch", epoch, "Loss:", loss.item())
...
student = PolicyNet(hidden_dim=32)

print("Distilling student using uniform noise only...")
