...
def train_teacher(cfg, env, logger, run_dir):
    print(f"Starting SB3 TRPO Teacher Training on {cfg.env.name}...")
    ...
    model = TRPO(
        ...
        batch_size=getattr(cfg.algo, "batch_size", 128),
        gamma=cfg.algo.gamma,
        cg_max_steps=getattr(cfg.algo, "cg_max_steps", 15),
        cg_damping=getattr(cfg.algo, "cg_damping", 0.1),
        line_search_shrinking_factor=getattr(cfg.algo, "line_search_shrinking_factor", 0.8),
        line_search_max_iter=getattr(cfg.algo, "line_search_max_iter", 10),
