...
class LinearLRSchedule:
    """
    A linear learning rate scheduler that decays the learning rate from its initial value
    to 0 over a specified number of updates.
    """
    ...
    def step(self):
        """
        Steps the scheduler, updating the optimizer's learning rate according to the schedule.
        """
...
class PPOLogger:
    """
    A logger utility for storing and visualizing PPO training metrics.
    Can optionally log data to TensorBoard.
    """
    ...
    def log_rollout_step(self, infos: dict, global_step: int):
        """
        Logs episode returns and lengths from vector environments during rollouts.

        Args:
            infos (dict): The info dictionary returned by `envs.step()`.
    ...
    def log_policy_update(self, update_results: dict, global_step: int):
        """
        Logs multiple loss metrics after a PPO update step.

        Args:
            update_results (dict): Dictionary of computed losses and stats.
            global_step (int): The current global training step count.
        """
        if self.use_tensorboard:
            self.writer.add_scalar(
                "losses/policy_loss",
                update_results["policy_loss"],
                global_step,
            )
            self.writer.add_scalar(
...
class PPO:
    ...
    def create_lr_scheduler(self, num_policy_updates):
        return LinearLRSchedule(
            self.optimizer, self.initial_lr, num_policy_updates
        )
    ...
    def learn(self, total_timesteps):
        ...
        """
        num_policy_updates = total_timesteps // (
    ...
    def _initialize_environment(self):
        ...
        is_initial_observation_terminal = torch.zeros(self.num_envs).to(
            self.device
        )
    ...
    def collect_rollouts(self, next_observation, is_next_observation_terminal):
        ...
        for step in range(self.num_rollout_steps):
            ...
            with torch.no_grad():
                action, logprob = (
                    self.agent.sample_action_and_compute_log_prob(
                        next_observation
                    )
                )
                value = self.agent.estimate_value_from_observation(
                    next_observation
                )
            ...
            next_observation, reward, terminations, truncations, infos = (
                self.envs.step(action.cpu().numpy())
            )
            self._global_step += self.num_envs
            rewards[step] = torch.as_tensor(reward, device=self.device).view(
                -1
            )
            is_next_observation_terminal = np.logical_or(
    ...
    def _initialize_storage(self):
        ...
        rewards = torch.zeros((self.num_rollout_steps, self.num_envs)).to(
            self.device
        )
        is_episode_terminated = torch.zeros(
            (self.num_rollout_steps, self.num_envs)
        ).to(self.device)
        observation_values = torch.zeros(
            (self.num_rollout_steps, self.num_envs)
        ).to(self.device)
    ...
    def compute_advantages(
        ...
        for t in reversed(range(self.num_rollout_steps)):
            ...
            delta = (
                rewards[t]
                + self.gamma * next_values * episode_continues
                - values[t]
            )
    ...
    def update_policy(
        ...
        for epoch in range(self.update_epochs):
            ...
            for start in range(0, batch_size, self.minibatch_size):
                ...
                with torch.no_grad():
                    ...
                    approx_kl = (
                        (probability_ratio - 1) - log_probability_ratio
                    ).mean()
        ...
        explained_variance = (
            ...
            - np.var(actual_returns - predicted_values)
            / observed_return_variance
        )
    ...
    def calculate_policy_gradient_loss(
        self, minibatch_advantages, probability_ratio
    ):
        """
        ...
        policy_gradient_loss = -torch.min(
            unclipped_pg_obj, clipped_pg_obj
        ).mean()
    ...
    def calculate_value_function_loss(
        ...
        if self.clip_value_function_loss:
            ...
            unclipped_vf_loss = (
                new_value - computed_returns[minibatch_indices]
            ) ** 2
            ...
            clipped_value = (
                previous_value_estimates[minibatch_indices]
                + clipped_value_diff
            )
            ...
            clipped_vf_loss = (
                clipped_value - computed_returns[minibatch_indices]
            ) ** 2
            ...
            value_function_loss = (
                0.5
                * (
                    (new_value - computed_returns[minibatch_indices]) ** 2
                ).mean()
            )
